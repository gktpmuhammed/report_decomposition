...
from config import (
    ...
    PROCESSING_CONFIG,
    LOGGING_CONFIG,
    # INPUT_FILES, # No longer needed
    # OUTPUT_FILES, # No longer needed
    LOGS_DIR
)

# Define input and output files for INSPECT dataset
INPUT_FILES = {
    "inspect": Path("/mnt/nas/Data_WholeBody/INSPECT/Final_Impressions.csv")
}
OUTPUT_FILES = {
    "impressions": Path("output/inspect/inspect_impressions.json")
}
...
Return ONLY the JSON object:"""
    ...
    def decompose_report(self, row: pd.Series, processing_mode: str = 'impressions') -> Dict[str, Dict[str, str]]:
        """Decomposes a single report row into impressions."""
        impression_id = row.get('impression_id', 'UNKNOWN')
        
        impressions_text = row.get('impressions', '')

        # Ensure text is a string
        impressions_text = str(impressions_text) if impressions_text is not None and pd.notna(impressions_text) else ""

        decomposed_data = {"impressions": {}}

        # Process impressions
        if impressions_text:
            decomposed_data["impressions"]["Conclusion"] = impressions_text
            extracted_impressions = self._rate_limited_call(impressions_text, impression_id)
            
            # Validate impressions extraction
            if not extracted_impressions and len(impressions_text.split()) > 20:
                self.logger.warning(f"Failed to extract impressions for {impression_id} with substantial text")
    ...
    def _process_single_report(self, row_data: Tuple, processing_mode: str = 'impressions') -> Tuple[str, Dict[str, Dict[str, str]]]:
        ...
        try:
            self.logger.debug(f"Starting processing for patient {impression_id}")
            ...
            self.logger.debug(f"Completed processing for patient {impression_id} in {processing_time:.2f}s")
            return impression_id, decomposed_data
            ...
            self.logger.error(f"{error_type} processing {impression_id}: {str(e)}")
            return impression_id, {"impressions": {}}

    def process_reports(self, input_data, output_dir: str = "data", sample_size: Optional[int] = None, save_interval: int = 100, impressions_path: Optional[Path] = None, processing_mode: str = 'impressions'):
        ...
        if os.path.exists(final_impressions_path):
            ...
            with open(final_impressions_path, 'r', encoding='utf-8') as f:
                ...
                processed_patients.update(all_impressions.keys())
                        ...
                        with self._lock:
                            if patient_id not in processed_patients:
                                ...
                                if completed_since_last_save >= save_interval:
                                    ...
                                    try:
                                        self._save_results(all_impressions, final_impressions_path, processing_mode=processing_mode)
...
====================="""
    ...
    def _save_results(self, impressions: Dict, impressions_path: Path, processing_mode: str = 'impressions'):
        ...
        try:
            ...
            if os.path.exists(impressions_temp):
                os.remove(impressions_temp)
            raise

def main():
    """Main function to run the decomposition."""
    model_name = MODEL_CONFIG["name"]
    save_interval = 25  # Set batch size to 25
    
    # Setup logging
    logger = setup_logging(model_name)
    logger.info(f"Starting INSPECT dataset decomposition with model: {model_name}")
    ...
    # Process the entire INSPECT dataset
    try:
        decomposer.process_reports(
            str(INPUT_FILES["inspect"]),
